...
export default function FacultyProfilePage({
  ...
  const hasFullProfile = member.problem && member.approach && member.discoveries;
  ...
  return (
    <>
      ...
      <div className="section-inner py-8">
        <StickySidebar sections={hasFullProfile ? sidebarSections : sidebarSections.filter(s => !["projects", "awards", "team"].includes(s.id) || member.projects.length > 0 || member.awards.length > 0 || member.teamMembers.length > 0)} />
          ...
          {memberPubs.length > 0 && (
            <section id="publications">
              ...
              {memberPubs.length > 5 && (
                ...
                >
                  {showAllPubs ? "Show Less" : `Show All ${memberPubs.length} Publications`}
                  {showAllPubs ? (
