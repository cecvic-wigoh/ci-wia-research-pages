...
export default function PersonMosaic() {
  const [hoveredSlug, setHoveredSlug] = useState<string | null>(null);

  return (
    <section className="section" aria-labelledby="people-mosaic-heading">
      <div className="section-inner">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          {/* Left: Text */}
          <div>
            <h2
              id="people-mosaic-heading"
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-1.5">
            {people.map((member, index) => (
              <Link
                key={member.slug}
                href={`/research/people/${member.slug}`}
            ...
            {Array.from({ length: Math.max(0, 16 - people.length) }).map(
              (_, i) => (
                <div
                  key={`empty-${i}`}
                  className={`aspect-square rounded-sm bg-gradient-to-br ${
                    gradients[(people.length + i) % gradients.length]
                  } opacity-30`}
                />
              )
            )}
